...
The remark plugin finds glossary terms while Docusaurus compiles Markdown. It injects the theme component import only on pages that contain linked terms, so the plugin doesn't need a global client module.
...
This runs tsup to create ESM, CommonJS, and declaration outputs in `dist/`.
...
1. **loadContent**: Reads and validates the configured glossary JSON file
2. **contentLoaded**: Creates component data, adds the glossary route, and exposes plugin data
3. **getThemePath**: Exposes the `GlossaryTerm` theme component
4. **getPathsToWatch**: Watches the glossary file during development
